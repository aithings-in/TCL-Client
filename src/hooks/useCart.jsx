...
const CART_STORAGE_KEY = 'tcl-ecommerce-cart';
...
export const CartProvider = ({ children }) => {
  ...
  const updateQuantity = useCallback((variantId, newQuantity) => {
    setCartItems(prevItems => {
        ...
        if (itemToUpdate.variant.manage_inventory && newQuantity > itemToUpdate.variant.inventory_quantity) {
          // Here you could also throw an error or show a toast
          console.warn(`Cannot update quantity beyond available stock for ${itemToUpdate.product.title}`);
          return prevItems.map(item =>
            item.variant.id === variantId ? { ...item, quantity: item.variant.inventory_quantity } : item
          );
        }
